(function() {
    ...
    /* GLOWNY ZAMEK: THE GATEWAY TO PRO */
    window.devkit = {
        enablePro: function() {
    ...
    var proFeatures = [
        ...
        { id: 'quickspy', name: '🕵️‍♂️ QuickSpy HUD (Mobile/PC)', key: 'pro_mod_quickspy', color: '#ff5555' }
    ];
    ...
    window.addEventListener('unhandledrejection', function(event) {
        window.__devKitErrors.count++;
        var errMsg = 'Unhandled Promise Rejection';
        if (event && event.reason && event.reason.message) errMsg = event.reason.message;
        else if (event && event.reason) errMsg = event.reason;
        window.__devKitErrors.logs.push({
            type: 'PROMISE_REJECTION', message: errMsg, file: 'async', line: 'N/A', time: new Date().toLocaleTimeString()
        });
        __updateDevKitBadge();
    });
    ...
    console.log("--- Menu.js Version 9.3 (Core Unified - Safe Inject Edition) Loaded ---");
    ...
    window.addEventListener('beforeunload', function() {
        if (localStorage.getItem('pro_save_on_refresh') === 'true') {
            ...
            var spyHudEl = clone.querySelector('#quick-spy-hud'); if (spyHudEl) spyHudEl.remove();
        ...
        proFeatures.forEach(function(feat) {
            ...
            if (isActive) {
                ...
                btn.onclick = function() {
                    ...
                    if (feat.id === 'quickspy') { if (window.StartQuickSpy) window.StartQuickSpy(); }
                    if (feat.id === 'fps') {
    ...
    function runDarkModeLogic(isManual) {
        ...
        style.textContent = 'html { color-scheme: dark !important; } html, body { background-color: #121212 !important; color: #e0e0e0 !important; } input, textarea, select { background-color: #2a2a2a !important; color: #fff !important; border: 1px solid #555 !important; } #pro-menu, #pro-fab, #pro-fps-hud, #pro-qr-modal, #__vconsole { color-scheme: dark !important; }';
    ...
    /* --- 11. QUICKSPY HUD (INTEGRATED PAYLOAD) --- */
    var spyHud = d.getElementById('quick-spy-hud');
    if (!spyHud) {
        spyHud = d.createElement('div');
        spyHud.id = 'quick-spy-hud';
        spyHud.style.cssText = 'position:fixed;top:10px;left:5%;width:90%;max-width:400px;max-height:250px;background:rgba(15,15,20,0.98);border:2px solid #ff5555;border-radius:12px;box-shadow:0 10px 30px rgba(0,0,0,0.8);z-index:2147483647;display:none;flex-direction:column;font-family:sans-serif;color:#eee;overflow:hidden;backdrop-filter:blur(5px);transition: all 0.2s ease;';
        
        spyHud.innerHTML = `
            <div style="padding:10px;background:#1a1a24;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #333;user-select:none;">
                <span style="color:#ff5555;font-weight:bold;font-size:12px;letter-spacing:1px;">🕵️‍♂️ QUICKSPY HUD</span>
                <button id="spy-close" style="background:#ff5555;color:#fff;border:none;border-radius:5px;padding:2px 8px;font-size:11px;font-weight:bold;cursor:pointer;">X</button>
            </div>
            <div id="spy-results" style="padding:12px;overflow-y:auto;font-size:13px;line-height:1.5;flex:1;-webkit-overflow-scrolling:touch;">
                <span style="color:#888;font-style:italic;">Przytrzymaj i zaznacz tekst pytania na ekranie...</span>
            </div>
        `;
        d.body.appendChild(spyHud);
        d.getElementById('spy-close').onclick = function() {
            spyHud.style.display = 'none';
            window.getSelection().removeAllRanges();
        };
    }

    var spyTimeoutTimer = null;
    var spyLastQuery = "";

    d.addEventListener('selectionchange', function() {
        if (localStorage.getItem('pro_mod_quickspy') !== 'true') return;
        clearTimeout(spyTimeoutTimer);
        
        spyTimeoutTimer = setTimeout(function() {
            var selectedText = window.getSelection().toString().trim();
            if (selectedText.length > 5 && selectedText !== spyLastQuery && !spyHud.contains(d.activeElement)) {
                spyLastQuery = selectedText;
                spyHud.style.display = 'flex';
                var resultsArea = d.getElementById('spy-results');
                resultsArea.innerHTML = '<div style="color:#ffaa66;text-align:center;padding:5px;">⚡ Szukam odpowiedzi...</div>';
                
                var url = 'https://api.duckduckgo.com/?q=' + encodeURIComponent(selectedText) + '&format=json&no_html=1';
                fetch(url)
                    .then(function(response) { return response.json(); })
                    .then(function(data) {
                        var html = "";
                        if (data.AbstractText) {
                            html += '<div style="background:rgba(85,255,85,0.1);border-left:3px solid #55ff55;padding:8px;margin-bottom:8px;border-radius:4px;"><b style="color:#55ff55;">Odpowiedź:</b><br>' + data.AbstractText + '</div>';
                        }
                        if (data.RelatedTopics && data.RelatedTopics.length > 0) {
                            var count = 0;
                            for (var i = 0; i < data.RelatedTopics.length; i++) {
                                if (data.RelatedTopics[i].Text && count < 2) {
                                    html += '<div style="margin-bottom:6px;padding-bottom:4px;border-bottom:1px solid #222;font-size:12px;">💡 ' + data.RelatedTopics[i].Text + '</div>';
                                    count++;
                                }
                            }
                        }
                        if (!html) {
                            html = '<div style="color:#ff5555;text-align:center;font-size:12px;">Brak prostej definicji.<br><a href="https://www.google.com/search?q=' + encodeURIComponent(selectedText) + '" target="_blank" style="color:#61afef;text-decoration:none;font-weight:bold;">Otwórz Google 🔍</a></div>';
                        }
                        resultsArea.innerHTML = html;
                    })
                    .catch(function(err) {
                        resultsArea.innerHTML = '<div style="color:#ef5350;text-align:center;">Błąd sieci.</div>';
                    });
            }
        }, 600);
    });
    ...
    window.StartQuickSpy = function() {
        spyHud.style.display = 'flex';
        d.getElementById('spy-results').innerHTML = '<span style="color:#888;font-style:italic;">Tryb szpiegowski aktywny. Zaznacz pytanie palcem lub myszką...</span>';
    };
    ...
    btnSaveRefresh.onclick = function() {
        if (localStorage.getItem('pro_save_on_refresh') !== 'true') {
            if (confirm("Activate 'Save on refresh'? This will freeze the HTML DOM structure across reload cycles.")) {
                ...
                var sH = clone.querySelector('#quick-spy-hud'); if(sH) sH.remove();
                localStorage.setItem('pro_persisted_html', clone.innerHTML);
                location.reload();
            }
        } else {
